...
from src.quantization import  wush_quantization_gptq
from src.utils.common_utils import fix_seed, clear_device_cache
...
def export_quantized_model(model, quantized_state_dict, non_quantized_state_dict, args):
        ...
        for k, v in block.state_dict().items():
            ...
            if f"{prefix}{layer_name}" in quantized_state_dict and param_name == "weight":
                for k_compr, v_compr in quantized_state_dict[f"{prefix}{layer_name}"].items():
                    ...
                    print(f"Saved quantized param: {prefix}{layer_name}.{k_compr}, shape: {v_compr.shape}, dtype: {v_compr.dtype}")
            elif f"{prefix}{k}" in non_quantized_state_dict:
                model_state_dict[f"{prefix}{k}"] = non_quantized_state_dict[f"{prefix}{k}"].cpu()
            else:
                print(f"Saving non-quantized param: {prefix}{k}, shape: {v.shape}, dtype: {v.dtype}")
                if "activation_quantizer" not in f"{prefix}{k}":
    ...
    config.quantization_config = prepare_quantization_config(
        ...
        transform_init="wush" if args.transform_class == "wush" else  args.transform_class
    )
    # Save configs
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="identity",
        choices=["identity", "hadamard", "wush"],
    ...
    parser.add_argument("--eval_platinumbench", action="store_true", help="whether to eval OpenLLM v1 openllm after quantization.")
    # LM eval params
    parser.add_argument(
        "--lm_eval_batch_size",
        type=auto_or_int,
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=["mmlu_cot_llama","mmlu","humaneval", "humaneval_instruct", "arc_challenge_llama", "gsm8k_llama", "hellaswag", "winogrande", "truthfulqa"],
...
def main():
    ...
    if quantize_anything:
        ...
        quantized_state_dict, non_quantized_state_dict = wush_quantization_gptq(model, calibration_data, args, device)
    ...
    if args.eval_platinumbench:
        if args.log_wandb:
            uniquename = (wandb.run.name + str(args.seed)).replace("/", "--")
            print(f"Platinumbench unique name: {uniquename}")
        else: 
            uniquename = str(args.seed)
        platinumbench.run_benchmark(model_list={uniquename:(model,tokenizer)}, output_file=None, parallelism=1, errors_dir=None, seed=args.seed, wandb=wandb if args.log_wandb else None, args=SimpleNamespace())
