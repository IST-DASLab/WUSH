...
# matmul_mxf8_bf16_nn is only compiled for sm_100 (Blackwell datacenter); see the
# `#if TARGET_CUDA_ARCH == 100` guard in qutlass/csrc/gemm.cu. On other GPUs it
# raises "Unsupported CUDA arch", so the GEMM portion of the fp8-requant test is
# skipped there (the requant/transpose checks still run).
_MXF8_NN_SUPPORTED = (
...
def _fp8_requant_test() -> None:
    ...
    if not _MXF8_NN_SUPPORTED:
        print("FP8 requant/transpose OK; SKIP matmul_mxf8_bf16_nn "
              "(requires sm_100 Blackwell datacenter, not built for this GPU)")
        return
