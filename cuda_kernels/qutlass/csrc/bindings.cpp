...
std::tuple<torch::Tensor, torch::Tensor> fusedQuantizeWushMxAbsMax(torch::Tensor const& A,
                                                                   torch::Tensor const& B,
                                                                   torch::Tensor& OUT,
                                                                   torch::Tensor& OUT_sf)
{
    torch::checkAllContiguous("fusedQuantizeWushMxAbsMax", {{A, "A", 0},
                                                        {B, "B", 1},
                                                        {OUT, "OUT", 2},
                                                        {OUT_sf, "OUT_sf", 3}});
    torch::checkDeviceType("fusedQuantizeWushMxAbsMax", {A, B, OUT, OUT_sf}, at::DeviceType::CUDA);
    torch::checkAllSameGPU("fusedQuantizeWushMxAbsMax", {{A, "A", 0},
                                                     {B, "B", 1},
                                                     {OUT, "OUT", 2},
                                                     {OUT_sf, "OUT_sf", 3}});
    TORCH_CHECK(A.scalar_type() == at::kBFloat16, "A must be bf16");
    ...
    uint32_t HAD_GS = 32; //B.size(-1);
    //TORCH_CHECK((A.numel()%HAD_GS)==0, "A must be divisible by", HAD_GS);

    //if(HAD_GS==32){
    fusedQuantizeWushMxAbsMax_host(OUT, OUT_sf, A, B);
    //} else {
    //    TORCH_CHECK(false,
    //                "Unsupported rotation size ", HAD_GS,
    //                "; expected 32.");
...
)
{
    ...
    m.def("fusedQuantizeWushMxAbsMax", &QUTLASS::fusedQuantizeWushMxAbsMax, "fusedQuantizeWushMxAbsMax");
